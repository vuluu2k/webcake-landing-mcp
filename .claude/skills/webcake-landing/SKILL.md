...
8. Give the editor URL for review. Note: `preview_url` on the preview host serves the STORED `app`/`app_css` build for only ~10 minutes after the last save — an MCP-created page's preview is **blank** until `publish_page` runs, and the link expires after the window. `publish_page({ page_id, dry_run:false })` builds + publishes (live PagePublishedV2 record); only `publish_page({ page_id, custom_domain, custom_path?, dry_run:false })` gives a permanent public URL — without a domain, tell the user the preview link is temporary and suggest attaching one.
