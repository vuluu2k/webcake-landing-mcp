...
`ingest_html(html, intent?, detail?)` / `ingest_url(url, intent?, detail?)` — parse HTML or fetch a URL into a reference AST (title, description, sections classified by role — hero/features/form/cta/footer/… — with headings, CTAs, images, form fields, top colors + fonts from BOTH inline styles and `<style>` blocks, CSS custom-property palette (design tokens by name), and background_images from stylesheets). `detail:'compact'` (default, ~2-5 KB) gives backward-compatible layout hints. `detail:'full'` (~up to 25 KB) adds: per-section `blocks` (repeating card/tile/step structures: title, body, image, cta), `lists` (li items), `gradients`, and `images` as `{ src, alt }` objects — use for clone-faithful rebuilds. Default `intent='adapt'` (rewrite content for user's brand); `intent='clone'` only when the user explicitly asks. When cloning: re-host image URLs found in the AST (`images`, `background_images`, `og_image`) via `upload_images` instead of hotlinking. For a screenshot/image input, no tool is needed — Claude analyzes it natively.
...
`publish_page` makes a page LIVE: calls the Webcake build host (`POST <buildBase>/render/build`, prod default `https://build.webcake.io`) to produce rendered `app`/`app_css` HTML, then saves a new source version + creates/updates `page_published` (optional `custom_domain`/`custom_path`). **An MCP-created page's `/preview/<page_id>` is blank until `publish_page` (with a build host) runs or the page is re-saved in the Webcake editor** — the preview host serves the STORED `app`/`app_css` build columns, not raw source. Without a build host `publish_page` falls back to source-only (`rendered:false`) with a warning. Set `WEBCAKE_BUILD_BASE` env or `x-webcake-build-base` header for staging/local (prod preset auto-configures). After a successful `publish_page` with build: `preview_url` renders immediately.
...
- failed/timed-out `create_page` → returns `draft_id` (full source cached ~30 min) → `patch_page({ draft_id, patches, dry_run:false })` fixes bad elements and creates the page; or `patch_page({ draft_id, dry_run:false })` with NO patches commits as-is (timeout retry).
- `add_section` dry_run=true, validation failure, or timeout → returns `draft_id` (section batch cached ~30 min) → `patch_page({ draft_id, patches, dry_run:false })` fixes elements and appends; or `patch_page({ draft_id, dry_run:false })` with no patches to commit as-is.
