...
- Re-check on MOBILE: the same string wraps MORE on the ${CANVAS.mobileWidth}px canvas — redo the math per breakpoint, don't copy the desktop height.
...
- CARD ANATOMY: the only generic container is group — rectangle/image-block/text-block are LEAVES and must NEVER have children (validation blocks it). Build a card as a group sized to the card box whose FIRST child is a full-size rectangle (top:0, left:0, width/height = the group's) carrying the card's background/borderRadius/boxShadow, followed by the icon/title/text children — children's top/left are RELATIVE to the group. Do NOT put background on the group itself: a group's own background/border does not render on the live page.
...
- HERO — always a clear H1, a short supporting line, and the primary CTA visible without scrolling. TWO-COLUMN HERO (text beside image): the text column must NEVER run under the image — every text element on the text side must satisfy left + width ≤ image.left − 24 on EACH breakpoint (e.g. image at left 560 → text width ≤ 536 − text.left). If the H1 needs more room, shrink/lower the image or shorten the headline — never let the headline slide behind the image.
- FEATURES / BENEFITS — a row of equal cards (icon + title + text) or a 2-column list. Center the row with the ROW math; on mobile shrink to one canvas width or stack. ICONS: do NOT use keyboard emoji (🎯💼📱✅⭐) as feature icons — instead use a small rectangle (e.g. 48×48px) with per-breakpoint config.svgMask set to a raw SVG string and styles.background set to the brand accent color; write a simple inline SVG path per icon (e.g. a checkmark, star, or arrow). Emoji are acceptable only inline within sentences, never as standalone card icons.
