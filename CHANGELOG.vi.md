...
### Added
- `search_images` nay trả về một `sizes` map trên mỗi ảnh (kích thước width/height thực tế theo px của từng biến thể src, có tính đến `dpr` cho màn hình retina) cùng trường `size_guide` giải thích cách khớp chiều rộng biến thể với chiều rộng hiển thị của ô chứa, để model chọn `src.large`/`src.large2x` cho hero, `src.medium` cho card/thumb, và `src.tiny` cho avatar thay vì đoán mò — tránh ảnh bị vỡ khi phóng to biến thể quá nhỏ và trang bị nặng vì dùng biến thể quá lớn trong ô nhỏ.

### Changed
- Quy tắc IMAGES trong generation guide nay hướng dẫn khớp kích thước biến thể với ô chứa (dùng dữ liệu `sizes`/`size_guide` mới và chiều rộng gốc của từng ảnh) thay vì quy tắc chung chung trước đây "large cho hero, medium cho card".

## [1.4.0] - 2026-07-23

### Changed
- `publish_page` nay tự động dùng lại domain đang gắn với trang khi bỏ trống `custom_domain` — giống cách modal publish của editor hoạt động, gọi `publish_page({ page_id })` đơn giản để publish lại sẽ giữ trang sống ở domain hiện có thay vì âm thầm rơi về link `/preview/<page_id>` tạm thời; domain được đọc từ `get_page_source`, nếu không có thì tra cứu qua `find_pages` theo id, và phản hồi dry-run nay ghi chú khi domain được kế thừa theo cách này. Truyền `custom_domain: ''` để chủ động ép publish không gắn domain.

## [1.3.0] - 2026-07-17

### Added
- `upload_images` và bước tự động re-host ảnh khi lưu trong `create_page`, `update_page`, `add_section`, `patch_page` nay lưu ảnh đã upload vào bộ sưu tập (media collection) của tổ chức — cùng thư viện mà trình chọn ảnh của editor đọc — tạo một bản ghi `Asset` để ảnh có thể được chọn lại trong editor, thay vì chỉ đẩy dữ liệu lên CDN công khai; phản hồi đánh dấu các mục này `collection:true` kèm `asset_id` (`upload_images`) hoặc `rehost.collection`/`rehost.collection_org_id` (đường lưu trang).
- `upload_images` có thêm tham số `in_folder` và `organization_id` để nhắm vào một thư mục cụ thể trong bộ sưu tập hoặc bộ sưu tập của một tổ chức cụ thể.

### Changed
- `upload_images` nay yêu cầu tổ chức phải được xác định trước khi lưu vào bộ sưu tập: nếu tài khoản có từ 2 tổ chức trở lên mà chưa chọn, tool trả về `ok:false` với `reason:"organization_required"` cùng danh sách tổ chức, giống cách `create_page` hoạt động; tài khoản chỉ có một tổ chức vẫn được tự động chọn, còn tài khoản không có thông tin xác thực vẫn rơi về endpoint CDN công khai như cũ.
- Ảnh upload qua đường bộ sưu tập nay được đặt tên theo URL/tên file nguồn thay vì tên chung chung `upload.<ext>`.
- Instruction của server nay yêu cầu người dùng cho phép (approve) bộ công cụ của server một lần duy nhất, ngay từ đầu, trong lúc intake (để tránh việc mỗi lần gọi tool đều bị hỏi quyền làm gián đoạn quá trình dựng trang), và chuyển bước xác định tổ chức mục tiêu qua `list_organizations` lên đầu intake, trước khi thực hiện bất kỳ công việc ảnh hoặc dựng trang nào.

## [1.2.1] - 2026-06-21

### Changed
- Instruction của server và `get_generation_guide` nay giới hạn bước visual-check chỉ trong ba trường hợp cụ thể — clone trang tham chiếu để so sánh độ trung thực pixel, người dùng yêu cầu xem hoặc xác nhận giao diện, hoặc có nghi ngờ thẩm mỹ cụ thể mà `validate_page` không thể phán xét — nên model không còn chụp màn hình mặc định sau mỗi lần build; khi cần chụp, một lần chụp duy nhất thay thế hướng dẫn lặp hai đến ba vòng trước đây, giảm token và độ trễ không cần thiết trong các build trang thông thường.

## [1.2.0] - 2026-06-21

### Added
- Tool mới `layout` tính toán chính xác tọa độ `top/left/width/height` cho cả hai breakpoint để model không cần tự tính `left`/`top` bằng tay (nguyên nhân hàng đầu gây lỗi lệch tâm); bốn chế độ — `center` (một hộp căn giữa canvas), `row` (N hộp xếp hàng ngang căn giữa trên desktop và tự động xếp chồng thành cột đơn trên mobile), `grid` (N ô đồng đều theo `cols` cột, block căn giữa; xếp chồng trên mobile), và `stack` (danh sách dọc theo cột nội dung chung trên cả hai breakpoint) — tuân theo trục page-margin (cột nội dung 80..880 desktop / 20..400 mobile theo mặc định), hỗ trợ canvas 960/1200 và 420/360, nhận căn lề trái/phải, và gắn cờ đầu vào nằm ngoài canvas hoặc quá rộng trong `notes`; tool là phép toán thuần túy, không yêu cầu mạng hay biến môi trường.

### Fixed
- Trường `serverInfo.version` trong MCP luôn báo cứng `"1.0.0"` tới mọi client bất kể phiên bản package đã cài; nay đọc phiên bản thực từ `package.json` lúc khởi động, để các MCP client và registry connector luôn thấy đúng phiên bản.

## [1.1.0] - 2026-06-20

### Added
- `create_page`, `add_section` và `validate_page` nay tự động áp dụng một lần auto-fix layout xác định trước khi validate hoặc lưu: các phần tử con nằm ngoài canvas được kéo về trong canvas (left được kẹp vào `[0, canvasW − width]`; `top` âm được đặt về 0); chiều cao `text-block` bị thu nhỏ được resize theo số dòng thực tế ước tính dùng cùng số liệu font mà validator dùng để cảnh báo; các phần tử nằm bên dưới một text-block đã reflow được đẩy xuống để tránh tràn; và các container cha được kéo dài để chứa đủ nội dung đã sửa — bản sửa mang tính bảo thủ (chỉ thêm khoảng trắng hoặc kéo hộp vào trong, không bao giờ di chuyển các phần tử được xếp lớp có chủ ý như badge overlay hay backdrop), idempotent, và chỉ áp dụng cho các công cụ tạo trang mới (`update_page` và `patch_page` được cố tình loại trừ để tránh di chuyển phần tử trên các trang hiện có).
- Mọi chỉnh sửa layout do `create_page`, `add_section` hoặc `validate_page` thực hiện nay được hiển thị trong trường phản hồi `auto_fixed` mới cùng với chỉ thị `auto_fixed_notice`, thay thế vòng lặp validate→cảnh báo→`patch_page`→re-validate trước đây bằng không cần round-trip nào cho hai lớp lỗi này trên các lần tạo trang mới.

## [1.0.84] - 2026-06-17

### Added
- `create_page` nay gửi thêm trường `by_ai` trong body của request tạo trang (giá trị mặc định `"mcp"`) để backend có thể đánh dấu từng trang được tạo qua MCP vào cột `by_ai`; đặt biến môi trường `WEBCAKE_BY_AI` để ghi đè tag này theo từng deployment.

## [1.0.83] - 2026-06-16

### Added
- Web guide của server (`GET /`) nay có thêm mục video hướng dẫn cài đặt song ngữ với các card theo từng ứng dụng (Claude, Codex, Antigravity); card có video đã ghi sẵn hiển thị thumbnail YouTube và mở embed `youtube-nocookie` bảo vệ quyền riêng tư trong lightbox modal khi nhấp; card chưa có video hiển thị placeholder "sắp có".
- Web guide của server (`GET /`) nay có thanh điều hướng sticky theo section, cho phép người đọc nhảy giữa các mục (Cách hoạt động, Tạo được gì, Copy trang, Kết nối, Video, Hỏi đáp) và tự động highlight mục hiện tại khi cuộn trang.

## [1.0.82] - 2026-06-16

### Added
- `render_preview` nay nhận tham số `tiles`: truyền `tiles:true` để nhận trang dưới dạng stack các ảnh dải ngang từ trên xuống dưới thay vì một ảnh full-page bị thu nhỏ đến giới hạn cạnh dài của model, giúp từng section hiển thị rõ nét ở độ phân giải đầy đủ; yêu cầu Playwright host tự host (`RENDER_SCREENSHOT_BASE`) và tự động fallback về ảnh full-page đơn khi chưa cấu hình.
- Các `serve` host nay chấp nhận `?tiles=1` (hoặc `?tiles=true`) trên route Playwright `GET /api/render/screenshot`, trả về JSON với mảng `tiles` (mỗi phần tử chứa `y`, `height`, và `data` base64) thay vì ảnh thô; tham số query `band_height` cho phép ghi đè chiều cao dải mặc định theo từng request.
- Biến môi trường mới `RENDER_SCREENSHOT_BAND_HEIGHT` đặt chiều cao dải tính theo CSS pixel mặc định cho engine tiling trên các `serve` host (mặc định 1400 px; tối thiểu 200).

## [1.0.81] - 2026-06-16

### Changed
- Engine chụp màn hình Playwright phục vụ route `GET /api/render/screenshot` của `render_preview` nay mặc định xuất ảnh JPEG thay vì PNG, giúp giảm kích thước payload base64 mà model nhận được khoảng 5–10× mà không ảnh hưởng đến khả năng so sánh layout và màu sắc; định dạng, chất lượng và mật độ điểm ảnh có thể điều chỉnh qua ba biến môi trường mới: `RENDER_SCREENSHOT_FORMAT` (`jpeg` hoặc `png`, mặc định `jpeg`), `RENDER_SCREENSHOT_QUALITY` (1–100, mặc định 72, chỉ áp dụng cho JPEG), và `RENDER_SCREENSHOT_SCALE` (deviceScaleFactor 0–2, mặc định 1).

## [1.0.80] - 2026-06-16

### Added
- Tool mới `render_preview` chụp màn hình `/preview/<id>` của một trang hoặc bất kỳ URL công khai nào và trả về ảnh PNG để model có thể NHÌN THẤY kết quả, cho phép vòng lặp so sánh trực quan và vá lỗi sau khi `create_page` hoặc `publish_page`; sử dụng Microlink làm engine mặc định miễn phí zero-config (~50 ảnh/ngày mỗi IP; trả về `ok:false` khi HTTP 429 để build loop bỏ qua bước kiểm tra trực quan thay vì thất bại), tự động chuyển sang engine Playwright tự host khi `RENDER_SCREENSHOT_BASE` (hoặc header per-request `x-render-screenshot-base`) được cấu hình; `MICROLINK_API_KEY` / `x-microlink-key` tăng hạn mức Microlink; `RENDER_SCREENSHOT_PRIMARY` kiểm soát engine nào được thử trước.
- Các `serve` host nay expose `GET /api/render/screenshot` — engine chụp màn hình Playwright (headless Chromium) mà `render_preview` chuyển sang từ Microlink khi `RENDER_SCREENSHOT_BASE` trỏ vào host; trả về `503` khi Playwright chưa được cài (`npm i playwright && npx playwright install --with-deps chromium` trên VPS, hoặc `playwright-core` + `CHROME_BIN` để tái sử dụng browser có sẵn của hệ thống); mặc định chặn các target private và loopback để phòng SSRF (tắt bằng `RENDER_ALLOW_PRIVATE=1`).

### Changed
- `get_generation_guide` và instruction server nay bổ sung bước VISUAL CHECK tường minh trong workflow xây dựng trang: sau khi `create_page` auto-publish hoặc sau `publish_page`, chụp màn hình trang đã render bằng khả năng browser của agent (ưu tiên, không giới hạn) hoặc qua `render_preview` làm fallback, so sánh với trang tham chiếu (thứ tự section, màu sắc, khoảng cách, vị trí ảnh, văn bản), `patch_page` từng mismatch theo id, publish lại và lặp 2–3 vòng cho đến khi trang khớp.
- Trang Privacy Policy tại `/privacy` được cập nhật để công khai rằng URL preview công khai của trang được gửi tới Microlink (hoặc renderer tự host) khi model thực hiện kiểm tra trực quan, và bổ sung Microlink vào danh sách dịch vụ bên thứ ba.

## [1.0.79] - 2026-06-15

### Changed
- Trang Privacy Policy tại `/privacy` được cập nhật với các tiêu đề phân loại dữ liệu theo chuẩn GDPR, ghi rõ mục đích xử lý từng loại dữ liệu, bổ sung tài liệu hóa PostgreSQL và Redis là các backend lưu trữ bền vững cho OAuth token và draft cache (với fallback về in-memory), mở rộng danh sách dịch vụ bên thứ ba bao gồm Iconify, và thêm mục "Dữ liệu chúng tôi KHÔNG thu thập" nêu rõ connector không bao giờ xử lý dữ liệu thẻ thanh toán, dữ liệu sức khỏe, số định danh chính phủ, bí mật xác thực, hay toàn bộ lịch sử hội thoại của người dùng.

## [1.0.78] - 2026-06-15

### Added
- Kho lưu trữ OAuth token (clients, authorization code, access và refresh token) nay sử dụng Postgres khi `DATABASE_URL`, `WEBCAKE_POSTGRES_URL`, hoặc `WEBCAKE_DATABASE_URL` được đặt: token tồn tại qua lần khởi động lại `serve` và được chia sẻ giữa các instance phía sau load balancer; các bảng cần thiết được tạo tự động lúc khởi động lần đầu; server tự động quay về store in-memory như trước khi không có Postgres, nên `serve` single-instance, stdio/`npx`, và smoke gate offline không cần thay đổi hạ tầng.
- Cache lưu trữ draft source dùng để khôi phục khi `create_page`, `update_page`, `add_section`, và `patch_page` gặp lỗi nay sử dụng Redis khi `REDIS_URL` hoặc `WEBCAKE_REDIS_URL` được đặt: draft tồn tại qua lần khởi động lại `serve` và được chia sẻ giữa các instance; server quay về in-memory khi không có Redis, và hành vi mất cache (model gửi lại source) không thay đổi với triển khai single-instance.
- Cache dedup image re-host nay sử dụng Redis khi `REDIS_URL` hoặc `WEBCAKE_REDIS_URL` được đặt: bảng ánh xạ `URL gốc → CDN URL` được chia sẻ giữa các instance `serve` và tồn tại qua khởi động lại, tránh upload lặp lại cùng một ảnh; quay về in-memory khi không có Redis, và cache miss chỉ đơn giản là upload lại thay vì gây lỗi.

## [1.0.77] - 2026-06-15

### Added
- Transport `serve` từ xa nay triển khai một OAuth 2.1 Authorization Server chuẩn spec (Authorization Code + PKCE S256, Dynamic Client Registration, refresh token, thu hồi token, và metadata `/.well-known` authorization-server và protected-resource), cho phép thêm server như một Claude custom connector hoặc ChatGPT plugin trực tiếp từ URL mà không cần sao chép JWT thủ công.
- Access token do OAuth cấp là chuỗi ngẫu nhiên mờ được ánh xạ phía server tới landing JWT Webcake của người dùng; các đường dẫn xác thực hiện có `?jwt=`, header `x-webcake-jwt`, và raw Bearer JWT tiếp tục hoạt động song song với OAuth token mà không có thay đổi nào.
- Trang Privacy Policy (`/privacy`) và Terms of Service (`/terms`) nay được phục vụ ngay trên origin của connector, đáp ứng yêu cầu URL công khai để nộp vào Claude Connectors Directory và ChatGPT App Directory.
- Web guide của server (trình duyệt `GET /`) nay có thêm mục "Clone" song ngữ giới thiệu việc clone từ URL trang web thật, màn hình Google Stitch, và file Figma; bổ sung icon copy/image/Figma vào bộ icon nội tuyến.

### Changed
- Transport `serve` nay bật OAuth enforcement theo mặc định: các yêu cầu `/mcp` không xác thực nhận `401 + WWW-Authenticate` để các client hỗ trợ OAuth (Claude, ChatGPT, MCP Inspector) tự động khởi động consent flow; đặt `WEBCAKE_OAUTH=0` để khôi phục hành vi truy cập mở như trước.
