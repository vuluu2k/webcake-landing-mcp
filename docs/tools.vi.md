...
# BỎ TRỐNG custom_domain để tái dùng domain đang gắn của trang (giống publish modal của editor);
# KHÔNG có domain thì chỉ còn /preview/<page_id> — link này hết hạn ~10 phút sau khi publish.
# Truyền custom_domain:"" để ép publish không domain. Không có build host: rơi về lưu chỉ-source.
publish_page({ page_id, dry_run: false })  # tái dùng domain đang gắn của trang
...
| Tool | Mô tả |
|------|-------------|
| `search_images` | Tìm ảnh stock THẬT (Pexels) cho trang — trả URL hotlink nhiều cỡ để gắn vào `specials.src` của element ảnh. **Chọn theo KÍCH THƯỚC, không chỉ chủ đề:** mỗi ảnh có map `sizes` (W×H px thực tế của từng biến thể) và response kèm `size_guide` — khớp bề rộng biến thể với bề rộng render của slot (hero → `src.large` ~940px / `src.large2x` ~1880px; card → `src.medium` ~350px; avatar → `src.tiny`), vì biến thể quá nhỏ kéo giãn ra slot lớn sẽ **vỡ ảnh**, còn `src.original` nhét vào card nhỏ sẽ làm **trang nặng**. Chạy **không cần setup** (proxy hosted chung cấp ảnh); đặt env `PEXELS_API_KEY` hoặc header `x-pexels-key` để dùng [key Pexels miễn phí](https://www.pexels.com/api/) / quota riêng. |
...
| `upload_images` | Chuyển URL ảnh ngoài (từ kết quả `ingest_html`/`ingest_url`), `data:` URI, hoặc **đường dẫn file cục bộ trên máy người dùng** (`/abs/path.jpg`, `~/Pictures/logo.png`, `file:///…`, `C:\…`) thành URL do Webcake host (`statics.pancake.vn`) để dùng trong `specials.src`. Truyền thẳng đường dẫn cục bộ vào `urls` — KHÔNG dùng dịch vụ bên thứ ba (catbox, imgur…) để chuyển file của người dùng. Sử dụng multipart upload (giới hạn backend 200 MB). Xử lý đến 20 entry/lần song song. **Lưu vào bộ sưu tập của người dùng:** khi có Webcake credentials VÀ đã resolve được org, ảnh được upload qua `POST <builderBase>/api/persona/upload` (đúng route mà media picker của editor dùng) — vừa lưu file vừa tạo bản ghi Asset, nên ảnh chọn lại được trong editor. Organization là **BẮT BUỘC** để upload vào bộ sưu tập, và phải ĐÚNG org sẽ tạo trang — hãy chốt org NGAY TỪ ĐẦU (lúc intake, trước mọi việc liên quan tới ảnh hoặc dựng trang). Thứ tự resolve: `organization_id` tường minh / `WEBCAKE_ORG_ID` / `x-webcake-org-id` được ưu tiên; tài khoản có ĐÚNG MỘT org → tự chọn; **có 2+ org mà không chọn → trả về `ok:false` với `reason:"organization_required"` kèm danh sách org, và phải gọi lại với `organization_id`** (giống hệt ràng buộc `create_page` đang áp dụng — không còn âm thầm lưu vào bộ sưu tập cá nhân nữa). Lưu được vào bộ sưu tập của org cần HAI thứ: header `x-org-id` VÀ `in_folder` = thư mục GỐC của bộ sưu tập org, do server resolve qua `GET <builderBase>/api/organization/folders/all?type=organization-image` (thư mục có `type === -1`) — chỉ gửi header thì asset bị kẹt ở thư mục cá nhân, không hiện ở thư viện nào cả. Chỉ khi THIẾU JWT mới fallback về endpoint công khai `POST <apiBase>/external/upload_file` — chỉ lưu bytes + trả URL CDN, không tạo Asset, `collection:false`, không cần credentials (đường zero-config cho `npx`, giữ nguyên như cũ). Tham số tùy chọn: `in_folder` (thư mục con cụ thể; ghi đè thư mục gốc org đã resolve; bị bỏ qua ở fallback công khai) và `organization_id` (org chứa bộ sưu tập cần lưu vào). Kết quả: mỗi entry `images[<url gốc>]` có thêm `collection: true|false` (+ `asset_id` khi đã lưu vào bộ sưu tập); cấp cao nhất có thêm `collection_uploads` (số lượng), `collection_org_id` và `collection_note`. **Upload thật theo mặc định (`dry_run` mặc định `false`)** — khác các tool persistence trang, tool này không đụng dữ liệu trang (nhiều nhất là thêm ảnh vào bộ sưu tập) nên mặc định là upload thật: chờ map `images`, rồi đặt từng `images[<url gốc>].url` vào mọi element đang dùng `<url gốc>` (slot upload thành công không bao giờ được dùng placeholder). Chỉ truyền `dry_run:true` khi muốn xem trước không-thao-tác (với file cục bộ, nó báo file có tồn tại không và kích thước). Đường dẫn cục bộ chỉ được phép khi server chạy ở chế độ stdio (máy cục bộ); trên HTTP transport từ xa sẽ bị từ chối theo từng entry. Ảnh trong reference là tài sản của người dùng — dùng tool này cho CẢ HAI intent (adapt VÀ clone) khi trang được dựng từ HTML/URL tham chiếu, khi người dùng cung cấp URL ảnh, hoặc khi người dùng cung cấp file ảnh trên máy; `search_images` chỉ lấp các slot không có ảnh nguồn. |
