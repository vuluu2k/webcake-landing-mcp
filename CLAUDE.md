...
**Auto-host external images on save.** Every REAL save — `createPage`/`updatePageSource`/`appendSection` in `webcake-client.ts` — runs `rehostSourceImages(config.base, source)` BEFORE the network store: it collects every external image URL in the tree (via [src/persistence/rehost.ts](src/persistence/rehost.ts)'s `collectExternalImageUrls` — `specials.src`, any `url(...)` background, gallery `item.link`, video poster; skips `data:`, already-hosted `statics.pancake.vn`, and placeholder hosts), downloads + re-uploads each to the Webcake CDN (`POST <apiBase>/external/upload_file`, the same public endpoint `upload_images` uses), and `rewriteImageUrls` swaps them in-tree. It's the chokepoint that makes the model's image step un-skippable: a clone just carries the source URLs through and they get hosted. Process-wide `rehostCache` dedupes across elements and saves; per-URL fetch/upload failure keeps the original URL and never blocks or throws; capped at `MAX_REHOST_PER_SAVE`. The outcome carries `rehost: {candidates,rehosted,failed,skipped}` + `rehosted_source` (the rewritten tree — `create_page`/`patch_page` feed it to auto-publish so the rendered build matches the stored tree). Dry-run never reaches these functions, so it never uploads. `rehost.ts` is PURE (no network/no `webcake-client` import → no cycle); the network pass lives in `webcake-client.ts`. So `upload_images` is now only REQUIRED for a user's LOCAL FILE PATHS (the save can't read local files).
