...
**Concurrency + the version-validated source cache** ([src/persistence/page-version.ts](src/persistence/page-version.ts)). `update_page` REPLACES the whole stored source, so a tree read earlier — or a `kind:'update'` draft that has sat for up to the 2-hour TTL — would silently revert whatever the user changed in the Webcake editor meanwhile. The backend's `update_page_source` is a blind `update_source(record, %{source: …})` with no version column, so the MCP does optimistic concurrency itself: it fingerprints the stored source (canonical key-sorted JSON → sha256) and keeps a per-page baseline, tagged `read` (bytes the backend handed us — a mismatch PROVES someone else edited) or `write` (our prediction after a save — a mismatch proves nothing, since the backend re-encodes the JSON). Before every whole-source overwrite `guardOverwrite` re-reads the page and calls the pure `decideOverwrite`: baseline matches → save; stale `read` baseline → refuse `page_changed_externally`; no/`write` baseline that would DROP live element ids → refuse `unverified_overwrite`; otherwise save with a notice. Refusals name `would_delete_element_ids` and point at `patch_page` (which merges into the tree it just read and can never revert anyone); `force:true` overrides. `get_page` returns `source_version` and `update_page`/`patch_page` accept `base_version`/`force`. `add_section` FORGETS the baseline — the backend appended server-side, so nothing we hold describes the stored tree.
The extra read is nearly free because it is version-validated, not cached blind (a cached freshness check is not a check): `GET /api/v1/ai/page_version` returns only `page_source.updated_at` — bumped by every save through `Pages.update_source/2`, the path BOTH the editor and `update_page_source` take — and `readLiveSource` reuses the cached tree only when that server-issued token is unchanged. It probes ONLY when something is cached, so a cold read is one request as before and a warm one is a few hundred bytes with no download; the cache is keyed by page id + a digest of the caller's JWT (safe under the multi-user HTTP transport) and bounded to a handful of pages. A backend without the route answers 404 → `updated_at` is absent → nothing ever caches → one full read, exactly as before.
...
- **No inline base64 images in a STORED source** — enforced by auto-upload, not by refusal. `rehost.ts` treats a `data:image/…;base64,…` URI as a re-host candidate (`isInlineBase64Image`/`parseInlineBase64Image` decode it, no fetch) and `webcake-client`'s `fetchAndHostOne` uploads the bytes on the same collection path as a remote image, so every real save rewrites base64 → a `statics.pancake.vn` URL (counted in `rehost`). `validate.ts` therefore only **warns** — a blocking error would stop the very save that fixes it; the warning exists to stop the model AUTHORING base64, which ships the whole image through the MCP link. Non-base64 `data:` URIs (e.g. `data:image/svg+xml;utf8,<svg…>`) are deliberately NOT touched, and `rehost-cache` hashes any key over 256 chars so a megabyte payload never becomes a Redis key.
