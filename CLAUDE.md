...
**media** (`search_images` — Pexels stock photos; needs a Pexels key but no Webcake env), **ingest**
...
`publish_page` makes a page LIVE: it reads the stored source and POSTs it to the editor's own `/api/pages/:page_id/edit/publish` route (NOT under `/api/v1/ai`), which saves a new version and creates/updates the `page_published` record with an optional custom domain/path. The **preview link does NOT need publishing** — `/preview/<page_id>` is served on the dedicated preview hosts (`preview.localhost:5800` local / `staging.webcake.me` staging / `www.webcake.me` prod, presets in `ENVIRONMENTS`; `WEBCAKE_PREVIEW_BASE` / `x-webcake-preview-base` override) where the v4 renderer reads the stored page_source directly. `toPreviewUrl` re-roots `preview_url` onto that host (`toEditorUrl` keeps editor links on the builder host); publishing source-only nils any editor-rendered head/body, so pages last published from the editor should be re-published there.
