...
The 16 tools fall into four groups: **reference** (`get_generation_guide`, `list_elements`, `get_element`,
...
**persistence** (`list_organizations`, `create_page`, `list_pages`, `find_pages`, `get_page`, `update_page`, `add_section`, `patch_page` — need env).
...
`patch_page` edits an EXISTING page by element id without re-sending the whole source — the model sends only per-element ops (`update`/`replace`/`remove`/`add`, keyed by id) and the MCP does the heavy lifting on the robust MCP↔backend link: it `get_page`s the live source, applies the ops in-tree, validates the WHOLE merged tree (blocks on errors), and `update_page`s. Unlike `add_section` there is no dedicated backend endpoint — it's a pure MCP-side get→merge→put, so it works against any backend. It needs creds even on `dry_run` (it must read the live page; `dry_run` only gates the write). Two uses: the SMALL surgical edit (change one element's text/color/position) and the FIX-AFTER-ERROR path (when `create_page`/`update_page`/`add_section` reports validation errors, correct only the offending element ids instead of rebuilding + re-shipping the whole payload).
